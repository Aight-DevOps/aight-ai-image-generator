...
# -*- coding: utf-8 -*-
"""
Streamlit Review Application - 美少女画像検品システム（メインアプリ）
"""
...
def main():
    ...
    st.set_page_config(
        page_title="美少女画像検品システム Ver7.2 - 自動データ更新対応版",
        page_icon="🖼️",
        layout="wide"
    )
    ...
    ui_components = UIComponents(review_system)
        ...
        if images_data:
            ...
            if len(images_data) > 0:
                ...
                if 'previous_image_id' not in st.session_state:
                    ...
                    # 前の画像の設定をクリア
                    ui_components.clear_comment_settings_on_image_change()
                    st.session_state.previous_image_id = current_image_id
                ...
                with col2:
                    # 拡張された画像メタデータの表示
                    ui_components.display_enhanced_image_metadata(selected_image)

                # 統合された時間帯別コメント・スロット設定エリア
                st.divider()
                comments, suitable_slots, recommended_slot = ui_components.render_integrated_comment_timeslot_area(selected_image)

                # 却下理由選択エリア
                st.divider()
                selected_reasons, other_reason = ui_components.render_rejection_reason_tags()
    ...
    with tab2:
        ...
        if stats:
            ...
            try:
                status_df = UIComponents.create_safe_dataframe(stats['status_counts'], "ステータス", "件数")
            ...
            try:
                highres_df = UIComponents.create_safe_dataframe(stats['highres_mode_counts'], "モード", "件数")
            ...
            try:
                genre_df = UIComponents.create_safe_dataframe(stats['genre_counts'], "ジャンル", "件数")
    ...
    with tab3:
        ...
        **🔄 ワークフロー**
        ...
        settings_data = {
            "AWS リージョン": review_system.s3_client.meta.region_name if hasattr(review_system, 's3_client') else "未設定",
            "S3 バケット": S3_BUCKET,
